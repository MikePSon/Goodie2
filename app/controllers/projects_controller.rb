class ProjectsController < ApplicationController
  ...
  private
    ...
    def project_params
      params.require(:project).permit(:name, :annual_budget, :organization_id,
        :repeat_type, :mission, :cycle_budget, :is_cycle, :is_project,
    ...
    def get_demo_info
      ...
      #FIXME: NEED APPLICANTS PROFILE
      #@all_applicants = User.where(:organization_id => current_user.organization_id.to_s).where(:applicant => true)
      @all_applicants = User.where(:organization_id => current_user.organization_id.to_s)
      ...
      @total_requested = @all_requests.sum(:amount_requested)
