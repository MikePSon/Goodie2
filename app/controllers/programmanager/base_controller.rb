class Programmanager::BaseController < ApplicationController
  ...
  def index
    ...
    if !@in_registration
		...
		@avg_accepted_rate = @closed_acceptance_rate.inject{ |sum, el| sum + el }.to_f / @closed_acceptance_rate.size
		@avg_incomplete_rate = @closed_incomplete_rate.inject{ |sum, el| sum + el }.to_f / @closed_incomplete_rate.size
		@avg_rejected_rate = @closed_rejection_rate.inject{ |sum, el| sum + el }.to_f / @closed_rejection_rate.size
		@avg_applicant_rate = (avg_application_total / closed_cycles.length).round(2)
		@avg_amount_given = @amount_given_array.inject{ |sum, el| sum + el }.to_f / @amount_given_array.size
